...
foreach (string name in studentNames)
{
    ...
    currentStudentExtraCreditGrade = (decimal)(sumExtraCreditAssignmentScores) / (studentScores.Length - examAssignments);
    extraCreditPoints = ((decimal)(sumExtraCreditAssignmentScores) / 10) / examAssignments;
    currentStudentGradeOverall = currentStudentGrade + extraCreditPoints;
    ...
    Console.WriteLine($"{currentStudent}\t\t{currentStudentGrade}\t\t{currentStudentGradeOverall}\t{currentStudentLetterGrade}\t{currentStudentExtraCreditGrade} ({extraCreditPoints} pts)");
}
